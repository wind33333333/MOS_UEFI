...
INIT_TEXT void init_gdt(void) {
    ...
    gdt_ptr.base = (UINT64*)LADDR_TO_HADDR(bitmap_alloc_pages((gdt_ptr.limit + 1) >> PAGE_4K_SHIFT));
    bitmap_map_pages(HADDR_TO_LADDR((UINT64)gdt_ptr.base),gdt_ptr.base,(gdt_ptr.limit + 1) >> PAGE_4K_SHIFT,PAGE_ROOT_RW);
