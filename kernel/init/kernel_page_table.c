...
INIT_TEXT void init_kpage_table(void) {
    ...
    //直接映射区
    memblock_mmap_range(kpml4t_ptr, 0,(void*)DIRECT_MAP_OFFSET,
    ...
    memblock_mmap_range(kpml4t_ptr, (uint64)_start_init_text - KERNEL_OFFSET, _start_init_text, _start_text - _start_init_text,
                        PAGE_ROOT_RWX_4K,PAGE_4K_SIZE);
    //.text可读执行
    memblock_mmap_range(kpml4t_ptr, (uint64)_start_text - KERNEL_OFFSET, _start_text, _start_data - _start_text,
                        PAGE_ROOT_RX_4K,PAGE_4K_SIZE);
    //.data-.stack可读写
    memblock_mmap_range(kpml4t_ptr, (uint64)_start_data - KERNEL_OFFSET, _start_data, _end_stack - _start_data, PAGE_ROOT_RW_4K,
                        PAGE_4K_SIZE);
    //设置正式内核页表
