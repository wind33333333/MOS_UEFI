...
INIT_TEXT void init_kpage_table(void) {
    ...
    memblock_vmmap1(kpml4t_ptr,_start_init_text-KERNEL_OFFSET, _start_init_text, _start_text - _start_init_text,
                    PAGE_ROOT_RWX);
    //.text可读执行
    memblock_vmmap1(kpml4t_ptr,_start_text-KERNEL_OFFSET, _start_text, _start_data - _start_text, PAGE_ROOT_RX);
    //.data-.stack可读写
    memblock_vmmap1(kpml4t_ptr,_start_data-KERNEL_OFFSET, _start_data, _end_stack - _start_data, PAGE_ROOT_RW);

    //memblock_vmmap1(PA_TO_VA(kpml4t_ptr),0,0xFFFF807FFFFFF000,2,PAGE_ROOT_RWX);
