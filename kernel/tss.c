...
INIT_TEXT void init_tss(void) {
    ...
    tss_t *tss_ptr = (tss_t *)LADDR_TO_HADDR(bitmap_alloc_pages(PAGE_4K_ALIGN(cpu_info.logical_processors_number * sizeof(tss_t)) >> PAGE_4K_SHIFT));
    bitmap_map_pages(HADDR_TO_LADDR(tss_ptr),tss_ptr,PAGE_4K_ALIGN(cpu_info.logical_processors_number * sizeof(tss_t)) >> PAGE_4K_SHIFT,PAGE_ROOT_RW);
    ...
    for (int i = 0; i < cpu_info.logical_processors_number; i++) {
        ...
        tss_ptr[i].rsp0 = (UINT64) LADDR_TO_HADDR(bitmap_alloc_pages(4) + PAGE_4K_SIZE * 4);
        bitmap_map_pages(HADDR_TO_LADDR(tss_ptr[i].rsp0)-PAGE_4K_SIZE * 4,(void*)(tss_ptr[i].rsp0-PAGE_4K_SIZE * 4),4,PAGE_ROOT_RW);
        tss_ptr[i].rsp1 = 0;
        tss_ptr[i].rsp2 = 0;
        tss_ptr[i].reserved1 = 0;
        tss_ptr[i].ist1 = (UINT64) LADDR_TO_HADDR(bitmap_alloc_pages(4) + PAGE_4K_SIZE * 4);
        bitmap_map_pages(HADDR_TO_LADDR(tss_ptr[i].ist1)-PAGE_4K_SIZE * 4,(void*)(tss_ptr[i].ist1-PAGE_4K_SIZE * 4),4,PAGE_ROOT_RW);
        tss_ptr[i].ist2 = 0;
        tss_ptr[i].ist3 = 0;
        tss_ptr[i].ist4 = 0;
        tss_ptr[i].ist5 = 0;
