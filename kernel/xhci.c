...
INIT_TEXT void init_xhci(void) {
    ...
    color_printk(GREEN,BLACK,"crcr:%#lx dcbaap:%#lx erstba[0]:%#lx erdp[0]:%#lx erstsz:%d config:%d ertrb[8].parameter1:%#lx ertrb[8].parameter2:%#x ertrb[8].parameter3:%#x\n",xhci_regs->op->crcr,xhci_regs->op->dcbaap,xhci_regs->runtime->intr_regs[0].erstba,xhci_regs->runtime->intr_regs[0].erdp,xhci_regs->runtime->intr_regs[0].erstsz,xhci_regs->op->config,xhci_regs->erdp_ptr[8].parameter1,xhci_regs->erdp_ptr[8].parameter2,xhci_regs->erdp_ptr[8].control);

    for (UINT32 i = 0; i < 10; i++) {
        color_printk(GREEN,BLACK,"ertrb[%d].parameter1:%#lx parameter2:%#x control:%#x\n",i,xhci_regs->erdp_ptr[i].parameter1,xhci_regs->erdp_ptr[i].parameter2,xhci_regs->erdp_ptr[i].control);
    }
