...
INIT_TEXT void init_xhci(void) {
    ...
    xhci_regs->crcr_ptr = kzalloc(trbs_number*16);          //分配命令环寄存器内存
    ...
    xhci_regs->erdp_ptr = kzalloc(trbs_number*16);  //分配事件环空间256*16
    ...
    for (UINT32 i=0;i<trbs_number;i++) {
        xhci_regs->crcr_ptr[i].control = 1;
        xhci_regs->erdp_ptr[i].control = 1;
    }
