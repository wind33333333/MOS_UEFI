...
uint8 xhci_handle_common_error(xhci_trb_comp_code_e comp_code, uint64 trb_pa) {
    ...
    switch (comp_code) {
        ...
        case XHCI_COMP_TRB_ERROR:
            color_printk(RED, BLACK, "[xHCI General Error] TRB Error (5) at PA %#llx: Invalid TRB format (Chain/Type wrong).\n", trb_pa);
            return 1;

        case XHCI_COMP_RESOURCE_ERROR:
            color_printk(RED, BLACK, "[xHCI General Error] Resource Error (7): Controller internal memory/resources exhausted.\n");
            return 1;
        ...
        case XHCI_COMP_EVENT_LOST_ERROR:
            color_printk(RED, BLACK, "[xHCI General Error] Event Lost (32): Event ring overflowed, hardware dropped events!\n");
            return 1;
...
int32 xhci_enable_slot(xhci_controller_t *xhci, uint8 *out_slot_id) { {
    ...
    void xhci_address_device(usb_dev_t *usb_dev) {
        ...
        slot64_t slot_ctx = {0};
        slot_ctx.route_speed = 1 << 27 | (xhci_controller->op_reg->portregs[usb_dev->port_id - 1].portsc & 0x3C00) <<
                               10;
